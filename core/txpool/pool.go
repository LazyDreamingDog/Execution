...
type BlockChain interface {
	...
	SubscribeChainHeadEvent(ch chan<- instance.ChainHeadEvent) event.Subscription
}
...
func (p *TxPool) loop(head *types.Header, chain BlockChain) {
	...
	var (
		newHeadCh  = make(chan instance.ChainHeadEvent)
...
func (p *TxPool) Add(txs []*Transaction, local bool, sync bool) []error {
	...
	for i, split := range splits {
		...
		if split == -1 {
			errs[i] = instance.ErrTxTypeNotSupported
			continue
		}
...
// func (p *TxPool) SubscribeNewTxsEvent(ch chan<- instance.NewTxsEvent) event.Subscription {
...
func (p *TxPool) Status(hash common.Hash) instance.TxStatus {
	for _, subpool := range p.subpools {
		if status := subpool.Status(hash); status != instance.TxStatusUnknown {
			return status
		}
	}
